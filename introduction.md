...
- 拿排队(同步)和叫号机(异步)来理解:
  ...
  Node.js 其实就是帮我们构建类似的机制。我们在写代码 ,发出异步请求的时候，实际上就是取号的过程，由 Event Loop 来接受处理，而真正执行操作的是具体的线程池里的 I/O 任务。之所以说 Node.js 是单线程，就是因为在接受任务(取号)的时候是单线程的，它无需进程/线程切换上下文的成本，非常高效，但它在执行具体任务(在线程池执行)的时候是多线程的。
